...
function App() {
  const [mode, setMode] = useState<'configurator' | 'grid'>('configurator')
  ...
  return (
    <div className="flex h-full w-full flex-col bg-zinc-950 text-zinc-100">
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3">
        <div className="text-sm font-medium text-zinc-200">Type Lab</div>
        <div className="flex items-center gap-2">
          <button
            className={`rounded-md px-3 py-1.5 text-sm ${mode === 'configurator' ? 'bg-zinc-100 text-zinc-950' : 'border border-zinc-800 bg-zinc-900 text-zinc-200'}`}
            onClick={() => setMode('configurator')}
          >
            Configurator
          </button>
          <button
            className={`rounded-md px-3 py-1.5 text-sm ${mode === 'grid' ? 'bg-zinc-100 text-zinc-950' : 'border border-zinc-800 bg-zinc-900 text-zinc-200'}`}
